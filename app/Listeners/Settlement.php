...
class Settlement implements ShouldQueue
{
    ...
    public function handle( SettlementEvent $event)
    {
        ...
        //查询昨日的总扣费情况
        $_price = ShopFlow::where('time',$this->time)
            ->where('flow_type',1)
    ...
    private function isEnouphFlow($shop_id,$time,$default_flow)
    {
        ...
        //计算到当日的流量总消耗
        $flow = ShopFlow::where(['remark' => 'flow', 'shop_id' => $shop_id, 'flow_type' => 0])
    ...
    private function isEnouphStorage($shop_id,$time,$default_storage)
    {
        ...
        //计算到当日的流量总消耗
        $flow = ShopFlow::where(['remark' => 'storage', 'shop_id' => $shop_id, 'flow_type' => 0])
            ->where('source',getenv('APP_ENV'))
            ->whereBetween('time', [$start_time, $end_time])
            ->sum('numberical');
