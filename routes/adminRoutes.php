...
Route::group([
    ...
    Route::group([
        ...
        Route::post('/delete', 'ColumnController@delete')->middleware('log')->name('columnDelete');
    ...
    Route::group([
        ...
        //结束直播
        Route::get('/end', 'AliveController@endLive')->middleware('log:id')->name('endLive');
    });
    ...
    Route::group([
        ...
        Route::post('/delete', 'CourseController@delete')->middleware('log')->name('courseDelate');
        ...
        // 课程目录结构
        Route::get('/{course_id}/struct', 'CourseStructController@struct')->name('courseStruct');

        //
        Route::post('/{course_id}/close_comment', 'CourseController@close_comment')->name('courseCloseComment');

        // 学习资料
        Route::get('/{course_id}/materials','CourseMaterialController@list')->name('courseMaterials');
        Route::post('/{course_id}/materials', 'CourseMaterialController@create')->name('CourseMaterialCreate');
        Route::get('/{course_id}/materials/{id}', 'CourseMaterialController@detail')->name('courseMaterialDetail');
        Route::put('/{course_id}/materials/{id}', 'CourseMaterialController@update')->name('courseMaterialUpdate');
        Route::post('/{course_id}/materials/bulk_delete', 'CourseMaterialController@bulkDelete')->name('courseMaterialBulkDelete');
        Route::post('/{course_id}/materials/{id}/order', 'CourseMaterialController@order')->name('courseMaterialOrder');

        // 学员
        Route::get('/{course_id}/students', 'CourseStudentController@list')->name('courseStudentList');
        Route::get('/{course_id}/students/export', 'CourseStudentController@export')->name('courseStudentExport');

        //试学列表
        Route::get('/{course_id}/previewers', 'CoursePreViewerController@list')->name('coursePreviewerList');
        Route::get('/{course_id}/previewers/export', 'CoursePreViewerController@export')->name('coursePreviewerExport');
        // course class set free
        Route::post('/{course_id}/classes/{class_id}/set_free', 'ClassController@setFree')->middleware('log')->name('classSerFree');
    ...
    */
    Route::group([
        ...
        Route::get('/list', 'PushRemindController@displayPush')->name('displayPush');
        Route::post('/open', 'PushRemindController@openPush')->name('openPush');
        Route::get('/test', 'PushRemindController@test')->name('openPushTest');
        Route::post('/push', 'PushRemindController@pushContents')->name('pushContents');
    });
...
Route::group([
    ...
    // 管理员回复删除
    Route::post('/reply/delete', 'CommentController@replyDelete')->name('replyDelete');
    //显示隐藏状态切换
    Route::get('/live/status', 'CommentController@changeStatus')->middleware('log:id')->name('changeLiveStatus');
    ...
    // 内容评分
    Route::get('/star', 'CommentStarController@star')->middleware('log:id')->name('commentStar');

    Route::get('/export', 'CommentExportController@export')->middleware('log:id')->name('commentExport');
    Route::post('/delete', 'CommentController@delete')->name('commentDelete');
});
...
Route::group([
    ...
    Route::group([
        ...
        Route::get('/list/', 'NoticeBannerController@lists')->name('noticeBannerLists');
    });
...
Route::group([
    ...
    Route::group([
        ...
        //订单数据汇总
        Route::get('/summary/search', 'OrderController@orderSummarySearch')->name('orderSummarySearch');
        Route::get('/check/status', 'OrderController@checkOrderStatus')->name('checkOrderStatus');
    });
...
Route::group([
    ...
    //店铺设置
    Route::post('/applet/setting', 'WebsiteController@shopSetting')->name('shopSetting');
...
//        Route::get('/sort', 'TypeController@sortType')->middleware('log:id')->name('navigationSort');
    ...
    Route::group([
        ...
        Route::get('/bill/list', 'FundsController@billSummaryList')->name('shopBillSummaryList');
        Route::get('/bill/detail', 'FundsController@dateBillDetail')->name('shopDateBillDetail');
        Route::get('/bill/detail/list', 'FundsController@dateBillList')->name('shopDateBillList');
        Route::get('/bill/list/export', 'FundsController@billExport')->name('shopBillExport');
        Route::get('/storage/export', 'FundsController@storageExport')->name('shopStorageExport');
        Route::get('/flux/export', 'FundsController@fluxExport')->name('shopFluxExport');
        Route::get('/balance', 'FundsController@balance')->name('shopFundsBalance');
    });
    ...
    Route::group([
        ...
        //分类排序
        Route::get('/sort', 'NavigationController@sortClass')->name('sortClass');
        //分类删除
    ...
    Route::group([
        ...
        //
        Route::get('/overview', 'DashboardController@overview')->name('overviewStatistics');
    });
...
//        Route::get('/check/status', 'WebsiteController@checkInfoStatus')->name('checkInfoStatus');
    });
});
...
Route::group([
        ...
        // 新建群发赠送码
        Route::post('/qunfazengsong', 'CodeController@createQunFazengSong')->middleware('log')->name('createQunFazengSong');
        // 群发赠送详情
        Route::get('/qunfazengsong/{id}', 'CodeController@qunFaZengSongDetail')->middleware('log')->name('qunFaZengSongDetail');
        //群发赠送会员领取列表
        Route::get('/qunfazengsong/{id}/receivers', 'CodeController@qunFaZengSongCodeList')->middleware('log')->name('qunFaZengSongCodeList');
        // 下载导入失败报表
        Route::get('/qunfazengsong/{id}/import_report', 'CodeController@downLoadCodeImportReport')->middleware('log')->name('selfMobileCodeImportReport');
        Route::get('/qunfazengsong/{id}/report', 'CodeController@exportQunfaCodesDetail')->middleware('log')->name('exportQunfaCodesDetail');
        //邀请码使用记录列表
...
Route::group([
    ...
    Route::group([
        ...
        Route::get('/info', 'AccountController@info')->name('accountInfo');
    });
});
...
Route::group([
    ...
    //内容不适用会员卡
    Route::post('/card/join','MemberCardController@contentJoin')->name('contentJoin');
});
...
Route::group([
    ...
    //返回登陆用户信息
    Route::get('/login/user', 'UserController@getLoginUserInfo')->name('loginUser');
});
...
Route::group([
    ...
    Route::get('/check/number','IndexController@checkNumber')->name('promotionCheckNumber');
    ...
    Route::post('/content/setting','ContentController@setting')->middleware('log')->name('contentSetting');
    ...
    Route::post('/setting','SetController@setting')->name('shopPromotionSetting');
    Route::group([
        'prefix'     => 'set',
//        'middleware' => 'promotion.shop'
    ],function () {
        Route::get('/check','SetController@setCheck')->middleware('log')->name('setCheck');
    ...
    Route::group([
        ...
        Route::get('/detail/{id}','PromoterController@detail')->name('promoterDetail');
        Route::get('/member/list','PromoterController@memberList')->name('promoterMemberList');
        Route::get('/record/list','PromoterController@recordList')->name('promoterRecordList');
    });
});
...
Route::group([
    ...
    //开通obs直播功能
    Route::get('/open', 'ObsController@openOBS')->middleware('log')->name('openOBS');
    Route::post('/refresh', 'ObsController@refreshObs')->middleware('log')->name('refreshObs');
    Route::get('/info', 'ObsController@obsInfo')->name('obsInfo');
    //开通在线直播
    Route::get('/online', 'ObsController@openOnlineLive')->middleware('log')->name('openOnlineLive');
...
Route::get('/content/cron/settlement','Setting\WebsiteController@settlement')->name('settlement');
